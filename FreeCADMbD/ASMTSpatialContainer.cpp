...
void ASMTSpatialContainer::initialize()
{
    ...
    axcms = FullRow<double>::With();
    aycms = FullRow<double>::With();
    azcms = FullRow<double>::With();
...
void MbD::ASMTSpatialContainer::updateMbDFromRotationMatrix(
    double v11,
    double v12,
    double v13,
    double v21,
...
{
    auto rotMat = std::make_shared<FullMatrix<double>>(
        ListListD {{v11, v12, v13}, {v21, v22, v23}, {v31, v32, v33}}
    );
...
void ASMTSpatialContainer::updateFromMbD()
{
    ...
    velocity3D = vOPO;
    auto aOPO = aOcmO->minusFullColumn(alpOPO->cross(rPcmO))
                    ->minusFullColumn(omeOPO->cross(omeOPO->cross(rPcmO)));
    acceleration3D = aOPO;
    ...
    axcms->push_back(aOcmO->at(0));
    aycms->push_back(aOcmO->at(1));
    azcms->push_back(aOcmO->at(2));
...
FColDsptr ASMTSpatialContainer::getCMAcceleration3D(size_t i) const
{
    auto vec3 = std::make_shared<FullColumn<double>>(3);
    vec3->atiput(0, axcms->at(i));
    vec3->atiput(1, aycms->at(i));
    vec3->atiput(2, azcms->at(i));
    return vec3;
}
